...
public class GenericSpecificationTest {
    ...
    private static final Logger LOG = LogUtil.getLogger(GenericSpecificationTest.class);

    private final String SPEC = GenericConfig.SPEC;
            ...
            for (Map.Entry<String, PathItem> entry : openAPI.getPaths().entrySet()) {
                ...
                for (Operation operation : pathItem.readOperationsMap().values()) {
                    for (String responseCode : operation.getResponses().keySet()) {
                        if (!responseCode.matches("^[0-9]+$") && !responseCode.matches("default")) {
                            String message = String.format(
